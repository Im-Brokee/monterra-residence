  ...
  return (
    <>
      <div className="catalogQuickFilters" aria-label="Szybki wybór liczby pokoi">
        <button className={rooms === "all" ? "active" : ""} onClick={() => setRooms("all")}>
          Wszystkie
        </button>
        {[1, 2, 3, 4, 5].map((value) => (
          <button
            key={value}
            className={rooms === String(value) ? "active" : ""}
            onClick={() => setRooms(String(value))}
          >
            {value} {value === 1 ? "pokój" : "pokoje"}
          </button>
        ))}
      </div>
      ...
      <section className="filters cardSoft expandedFilters">
        ...
        <label>
          ...
          <select value={sort} onChange={(e) => setSort(e.target.value)}>
            ...
            <option value="floor-asc">Piętro</option>
          </select>
        </label>

        <button onClick={reset} className="button buttonGhost filterReset">
          Wyczyść filtry
        </button>
      </section>
      ...
      <div className="resultBar catalogToolbar">
        <span>
          Znaleziono: <strong>{result.length}</strong>
        </span>
      ...
      {view === "list" && (
        <div className="catalogTableWrap cardSoft">
            ...
            <tbody>
              {result.map((a) => (
                <tr key={a.id}>
                  ...
                  <td>{a.floor}</td>
                  <td>
                    {a.balconyType === "garden"
                      ? "Ogród"
                      : a.balconyType === "terrace"
                        ? "Taras"
                        : a.balconyType === "balcony"
                          ? "Balkon"
                          : "—"}
                  </td>
                  <td>
                  ...
                  <td>{formatPln(a.price)}</td>
                  <td>
                    <Link href={`/mieszkania/${a.slug}`} className="tableLink">
                      Zobacz →
                    </Link>
                  </td>
                </tr>
